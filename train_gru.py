...
# Define the GRU model for multi-class classification
def create_gru_model(input_shape, num_classes, hparams):
    model = Sequential()
    model.add(GRU(hparams["HP_GRU_UNITS"], return_sequences=True, recurrent_dropout=0.2, input_shape=input_shape, kernel_regularizer=l2(hparams["HP_L2_LAMBDA"])))
    model.add(GRU(hparams["HP_GRU_UNITS"], return_sequences=False, recurrent_dropout=0.2, kernel_regularizer=l2(hparams["HP_L2_LAMBDA"])))
...
def run_trial(hparams, folds, use_pca):
    ...
    for i, (train_windows, train_labels, test_windows, test_labels) in enumerate(normalized_folds):
        if i >= folds:
            ...
            model = create_gru_model((window_size, train_windows.shape[-1]), num_classes, hparams)
            history = model.fit(train_windows, train_labels_encoded,
                    epochs=hparams["HP_EPOCHS"], batch_size=hparams["HP_BATCH"], validation_data=(test_windows, test_labels_encoded), shuffle=True, verbose=1)
...
if __name__ == "__main__":
    ...
    folds2Test = 1
    results = run_trial(hparams, folds2Test, True)
