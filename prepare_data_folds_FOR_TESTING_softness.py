...
window_size = 2
train_windows, train_labels = create_windows(data, window_size*sampling_freq, overlap=0)
...
# Define the window size
window_size = 2
test_windows, test_labels = create_windows(data, window_size*sampling_freq, overlap=0)

test_windows, test_scaler = normalize_windows(test_windows)
test_windows_pca = [pca.transform(window) for window in test_windows]
normalized_data = (train_windows_pca, train_labels, test_windows_pca, test_labels)

plot_pca([pca])

# One-hot encode labels
softness_encoder = OneHotEncoder(sparse_output=False)
softness = np.array(train_labels[:, 1]).reshape(-1, 1)
encoded_softness = softness_encoder.fit_transform(softness)

# Save normalized folds and scalers
joblib.dump(normalized_data, 'normalized_data_Softness_Train_Test.pkl')
joblib.dump(train_scaler, 'scalers_pca__Softness_Train_Test.pkl')
joblib.dump(test_scaler, 'testscalers_pca__Softness_Train_Test.pkl')
joblib.dump(softness_encoder, 'softnessencoder__Softness_Train_Test.pkl')
joblib.dump(pca, 'pca_Softness_Train_Test.pkl')
...
# train_scaler = joblib.load('scalers_pca_allData_peakTrim_text&soft.pkl')
# print(train_scaler)
# pca = joblib.load('pcas_pca_allData_peakTrim_justsoftness.pkl')[0]

# test_windows, _ = normalize_windows(windows, scaler=train_scaler[0])
# test_windows_pca = [pca.transform(window) for window in test_windows]
# normalized_data = [test_windows_pca, labels]
...
# labels = np.array(labels[:, 0].astype(int)).reshape(-1, 1)
# encoded_labels = labels_encoder.fit_transform(labels)

# # Save normalized folds and scalers
# joblib.dump(normalized_data, 'normalized_folds_pca_allData_peakTrim_text&soft_TEST.pkl')
# joblib.dump(encoded_labels, 'encoded_labels_pca_allData_peakTrim_text&soft_TEST.pkl')
# joblib.dump(labels_encoder, 'labelsencoder_pca_allData_peakTrim_text&soft_TEST.pkl')
# joblib.dump(encoded_softness, 'encoded_softness_pca_allData_peakTrim_text&soft_TEST.pkl')
# joblib.dump(softness_encoder, 'softnessencoder_pca_allData_peakTrim_text&soft_TEST.pkl')
