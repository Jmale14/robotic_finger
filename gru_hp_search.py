...
from train_gru import run_trial
...
folds2Test = 5
use_pca = True
HP_H_UNITS = hp.HParam('h_units', hp.Discrete([64]))
HP_GRU_UNITS = hp.HParam('gru_units', hp.Discrete([128]))
HP_EPOCHS = hp.HParam('epochs', hp.Discrete([1000]))
HP_BATCH = hp.HParam('batch', hp.Discrete([64]))
HP_LR = hp.HParam('lr', hp.Discrete([0.0001, 0.0005]))
...
TRIALS = len(HP_H_UNITS.domain.values) * \
    len(HP_GRU_UNITS.domain.values) * len(HP_EPOCHS.domain.values) * \
...
for h_units in HP_H_UNITS.domain.values:
    for gru_units in HP_GRU_UNITS.domain.values:
                ...
                for lr in HP_LR.domain.values:
                    for l2_lambda in HP_L2_LAMBDA.domain.values:
                        ...
                        for m in ['loss', 'accuracy', 'f1_score']:
                            name = f"train_hist_{m}_PCA{use_pca}_GRU_" + startTimeStamp
                        ...
                        name = f"trial_details_PCA{use_pca}_GRU_" + startTimeStamp
                        with open(f'{name}.csv', 'a') as out:
